...
class Windows(PlatformDirsABC):  # noqa: PLR0904
    ...
    def user_templates_dir(self) -> str:
        ...
        return os.path.normpath(
            str(
                Path(get_win_folder("CSIDL_APPDATA"))
                / "Microsoft"
                / "Windows"
                / "Templates"
            )
    ...
    def user_bin_dir(self) -> str:
        ...
        return os.path.normpath(
            os.path.join(get_win_folder("CSIDL_LOCAL_APPDATA"), "Programs")
        )  # noqa: PTH118
    ...
    def site_bin_dir(self) -> str:
        ...
        return os.path.normpath(
            os.path.join(get_win_folder("CSIDL_COMMON_APPDATA"), "bin")
        )  # noqa: PTH118
...
def get_win_folder_if_csidl_name_not_env_var(
    csidl_name: str,
    ...
    if csidl_name == "CSIDL_COMMON_PROGRAMS":
        return os.path.join(  # noqa: PTH118
            os.path.normpath(
                os.environ.get(
                    "PROGRAMDATA", os.environ.get("ALLUSERSPROFILE", "C:\\ProgramData")
                )
            ),
            "Microsoft",
            "Windows",
...
def get_win_folder_from_registry(csidl_name: str) -> str:
    ...
    key = winreg.OpenKey(
        hkey, r"Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders"
    )
